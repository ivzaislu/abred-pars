...
A source cursor advances only after its feed ZIP has been atomically published and registered. Feed ZIPs are immutable.
...
Schedule claims are persisted in SQLite, so restarting the process inside the same due minute does not create another scheduled run. Per-source file locks prevent overlapping runs across local processes.

Disable the built-in scheduler with `PARSER_SCHEDULER_ENABLED=false` if systemd timers or another host scheduler will be used.
...
RuTracker still uses the existing Worker transport and TorrServer enrichment. Server mode expects `RUTRACKER_WORKER_URL`. With `RUTRACKER_TORRSERVER_ENRICH=true` (the default), at least `TORRSERVER_URL` must be configured; `TORRSERVER_URL_2` remains optional for the existing pool/failover behavior.
